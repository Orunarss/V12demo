...
class RepairReport(models.AbstractModel):
    _name = 'report.abs_maintenance_summary.maintenance_report'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if docids:
            ...
            if docs:
                maintenance_ids = self.env['maintenance.request'].search([('request_date','>=',docs.start_date),('request_date','<=',docs.to_date),("user_id",'=',docs.employe_id.name)])
                if not maintenance_ids:
                    raise ValidationError("The maintenance has no request At this date interval")  
               
        return {
                   'doc_ids': maintenance_ids,
